...
struct JIT_LOG {
	...
	JIT_LOG(DWORD __maxsize): maxsize(__maxsize + 1), counter(0), abandoned(0), hout(GetStdHandle(STD_OUTPUT_HANDLE)),
		logdata((wchar_t*)HeapAlloc(__g_hheap, 0, maxsize * sizeof(wchar_t))) { }
	~JIT_LOG() {
	...
	void log(const wchar_t* format, ...) {
		...
		for (DWORD i = 0; i < len; i++) {
			switch (format[i]) {
				...
				default: logdata[counter++] = wchar_t(format[i]);
			}
		}
		va_end(args);
		logdata[counter] = L'\0';
...
struct JITvar {
	BYTE regID, state;
	DWORD stackID;
	wchar_t name[JIT::MAX_VAR_NAME];
	JITvar(BYTE state, BYTE regID, DWORD stackID, const wchar_t* __name): state(state), regID(regID), stackID(stackID) {
		wcscpy(name, __name);
	}
...
namespace JIT {
	...
	const wchar_t VOID_NAME[] = L"__void";
}
struct JITcode {
	BYTE  op, mode;
	wchar_t  var_source[JIT::MAX_VAR_NAME], var_dest[JIT::MAX_VAR_NAME], var_addition[JIT::MAX_VAR_NAME];
	union {
		float data_f;
		DWORD data_i;
		void* funcptr;
...
struct lsJITcode {
	...
	void newvar(const wchar_t* name) {
		wcscpy(code[counter].var_source, name); wcscpy(code[counter].var_dest, JIT::VOID_NAME);
		code[counter++].op = JIT::NEW;
	}
	void calc(BYTE __mode, const wchar_t* source, const wchar_t* dest) {
		wcscpy(code[counter].var_source, source);
		if (dest == NULL) wcscpy(code[counter].var_dest, source);
		else wcscpy(code[counter].var_dest, dest);
		code[counter].op = JIT::CALC; code[counter++].mode = __mode;
	}
	void setconst(const wchar_t* source, float floatconst, bool independent, const wchar_t* tag) {
		if (independent) {
			wcscpy(code[counter].var_addition, tag);
			code[counter].op = JIT::SETCONST_IDP;
		} else {
			code[counter].op = JIT::SETCONST;
			code[counter].data_f = floatconst;
		}
		wcscpy(code[counter].var_source, source); wcscpy(code[counter++].var_dest, JIT::VOID_NAME);
	}
	void callfunc(void* func, const wchar_t* result, const wchar_t* arg_1, const wchar_t* arg_2) {
		wcscpy(code[counter].var_addition, result); wcscpy(code[counter].var_source, arg_1);
		if (arg_2 == NULL) wcscpy(code[counter].var_dest, JIT::VOID_NAME);
		else wcscpy(code[counter].var_dest, arg_2);
		code[counter].op = JIT::CALLFUNC; code[counter++].funcptr = func;
	}
	void finish(const wchar_t* ret) { wcscpy(code[counter].var_source, ret); code[counter++].op = JIT::FINISH; }
};
...
struct lsJITvar {
	...
	lsJITvar(DWORD code_size, DWORD stack_size, DWORD max_vars, DWORD max_consts): var_counter(2), const_counter(0),
		...
		using namespace JIT;
		vars[0] = JITvar(AT_REG, 0, I_DONT_CARE, L"x");
		vars[1] = JITvar(AT_REG, 1, I_DONT_CARE, L"y");
	...
	DWORD find_var(const wchar_t* name) {
		for (DWORD i = 0; i < var_counter; i++) if (wcscmp(vars[i].name, name) == 0) return i;
		__log(L"(find ~)\n", name);
	...
	BYTE swap_back(const wchar_t* name, BYTE protected_regID) {
		using namespace JIT;
	...
	void new_var(const wchar_t* name) { wcscpy(vars[var_counter++].name, name); }
	void init_var(const wchar_t* name, BYTE regID) {
	...
	void __call_func_movarg(DWORD argID, BYTE regID) {
		...
		if (vars[argID].state == AT_STACK) {
			...
			} if (vars[argID].stackID == I_DONT_CARE) {
				vars[argID].stackID = stack_counter++;
	...
	void call_func(void* func, const wchar_t* result, const wchar_t* arg_1, const wchar_t* arg_2) {
    	using namespace JIT;
    	DWORD arg_1_ID = find_var(arg_1);
		DWORD arg_2_ID = (wcscmp(arg_2, VOID_NAME) != 0) ? find_var(arg_2) : I_DONT_CARE;
		{
	...
	BYTE* finish(const wchar_t* name) {
		using namespace JIT;
...
struct lsIDPCONST {
	...
	void append(const wchar_t* name, DWORD __stack_loc) {
		for (DWORD i = 0; i < counter; i++) if (wcscmp(consts[i].name, name) == 0) {
			consts[i].stack_loc = __stack_loc;
			return;
		}
		wcscpy(consts[counter].name, name);
		consts[counter++].stack_loc = __stack_loc;
	}
	float* getloc(const wchar_t* name, BYTE* EXEmem) {
		for (DWORD i = 0; i < counter; i++) if (wcscmp(consts[i].name, name) == 0)
...
void pre_compile(const lsJITcode* source, lsJITvar* dest, DWORD max_vars) {
	...
	for (DWORD i = 0; i < source->counter; i++) {
		for (DWORD j = 0; j < var_counter_pre; j++) if (wcscmp(vars_pre[j].name, source->code[i].var_source) == 0 ||
			wcscmp(vars_pre[j].name, source->code[i].var_dest) == 0) vars_pre[j].end = i;
		if (source->code[i].op == NEW) {
			wcscpy(vars_pre[var_counter_pre].name, source->code[i].var_source);
...
struct EXPRTOKEN {
	...
	union {
		float floatval;
		wchar_t name[__token_maxconst(JIT::MAX_FUNC_NAME, JIT::MAX_VAR_NAME)];
		wchar_t singlechar;
	};
};
struct lsEXPRTOKEN {
	...
	void append(BYTE type, const wchar_t* name, float floatval, wchar_t singlechar) {
		using namespace JIT;
		switch (type) {
			case TRUE_CONST: tokens[counter].floatval = floatval; break;
			case OPERATOR: tokens[counter].singlechar = singlechar; break;
			case VARNAME: case FUNCNAME: case FAKE_CONST: wcscpy(tokens[counter].name, name);
		}
		tokens[counter++].type = type;
	}
};

inline bool __expr2token_isnum(wchar_t c) { return (L'0' <= c && c <= L'9') || c == L'.'; }
inline bool __expr2token_isop(wchar_t c) { return c == L'+' || c == L'-' || c == L'*' || c == L'/' || c == L'^'; }
inline bool __expr2token_isabc(wchar_t c) { return (L'A' <= c && c <= L'Z') || (L'a' <= c && c <= L'z'); }
void expr2token(const wchar_t* expr, lsEXPRTOKEN* dest, lsIDPCONST* changeables) {
	using namespace JIT;
	DWORD exprlen = wcslen(expr);
	for (DWORD i = 0; i < exprlen; i++) {
		if (expr[i] == L' ') continue;
		if (__expr2token_isnum(expr[i])) {
			...
			do {
				if (expr[i] == L'.') {
					...
					afterdot = 1;
				} else if (afterdot) floatval += float(expr[i] - L'0') * (powval *= .1f);
				else floatval = floatval * 10.f + float(expr[i] - L'0');
				i++;
		...
		else if (__expr2token_isabc(expr[i]) || expr[i] == L'_') {
			bool spaced = 0, isfunc = 0;
			DWORD j = i, namelen = 1;
			while (++j < exprlen) {
				if (__expr2token_isabc(expr[j]) || __expr2token_isnum(expr[j]) || expr[j] == L'_') {
					if (spaced) break;
					namelen++; continue;
				} if (expr[j] == L' ') { spaced = 1; continue; }
				if (expr[j] == L'(') { isfunc = 1; break; }
				if (__expr2token_isop(expr[j])) break;
			}
			wchar_t name[MAX_VAR_NAME]; memcpy(name, expr + i, namelen * sizeof(wchar_t)); name[namelen] = L'\0';
			BYTE type = FAKE_CONST;
			if (isfunc) type = FUNCNAME;
			else if (wcscmp(name, L"x") == 0 || wcscmp(name, L"y") == 0) type = VARNAME;
			dest->append(type, name, NCM::NAN, NULLCHAR);
			if (type == FAKE_CONST) __log(L"(got independent const ~)\n", name);
			i += namelen - 1;
		} else if (expr[i] == L'(') dest->append(PAR_LEFT, NULL, NCM::NAN, NULLCHAR);
		else if (expr[i] == L')') dest->append(PAR_RIGHT, NULL, NCM::NAN, NULLCHAR);
		else if (expr[i] == L',') dest->append(COMMA, NULL, NCM::NAN, NULLCHAR);
...
namespace JIT { const wchar_t TEMPVAR_HEADER[MAX_VAR_NAME] = L"__tmp"; }
...
inline wchar_t __token2code_tempname_singlechar(DWORD* counter_copy) {
	BYTE remainder = *counter_copy % 62; // 62 = 26 + 26 + 10
	*counter_copy /= 62;
	if (remainder < 26) return L'A' + remainder;
	else if (remainder < 52) return L'a' + (remainder - 26);
	return L'0' + (remainder - 52);
}
void __token2code_tempname(wchar_t* dest, DWORD* strptr, DWORD* counter) {
	if (*counter == 0) dest[(*strptr)++] = L'A';
	else {
...
void __token2code_calc(wchar_t* result_name, wchar_t* tempvar, wchar_t lastest_op, lsJITcode* dest) {
	using namespace JIT;
	BYTE opJA;
	switch (lastest_op) {
		case L'+': opJA = ADD; break; case L'-': opJA = SUB; break;
		case L'*': opJA = MUL; break; case L'/': opJA = DIV; break;
		case L'^': {
...
struct JITFUNC {
	wchar_t name[JIT::MAX_FUNC_NAME];
	void* funcptr;
};
struct lsJITFUNC {
	...
	lsJITFUNC(DWORD max_funcs, bool avtivate_builtins): max_funcs(max_funcs), counter(0),
		...
		if (avtivate_builtins && max_funcs >= 21) {
			append(L"sin", (void*)NCM::sinf); append(L"cos", (void*)NCM::cosf); append(L"tan", (void*)NCM::tanf);
			append(L"csc", (void*)NCM::cscf); append(L"sec", (void*)NCM::secf); append(L"cot", (void*)NCM::cotf);
			append(L"asin", (void*)NCM::asinf); append(L"acos", (void*)NCM::acosf); append(L"atan", (void*)NCM::atanf);
			append(L"acsc", (void*)NCM::acscf); append(L"asec", (void*)NCM::asecf); append(L"acot", (void*)NCM::acotf);
			append(L"sinh", (void*)NCM::sinhf); append(L"cosh", (void*)NCM::coshf); append(L"tanh", (void*)NCM::tanhf);
			append(L"csch", (void*)NCM::cschf); append(L"sech", (void*)NCM::sechf); append(L"coth", (void*)NCM::cothf);
			append(L"abs", (void*)NCM::absf); append(L"log", (void*)NCM::logf2); append(L"log1", (void*)NCM::logf);
		}
	}
	~lsJITFUNC() {
	...
	void append(const wchar_t* name, void* funcptr) {
		wcscpy(funcs[counter].name, name);
		funcs[counter++].funcptr = funcptr;
	}
	void* find_func(const wchar_t* name) {
		for (DWORD i = 0; i < counter; i++) if (wcscmp(funcs[i].name, name) == 0) return funcs[i].funcptr;
		__report_error(L"Unknown func name"); return NULL;
	}
};
void __token2code_tempvar(
		lsEXPRTOKEN* source, lsJITcode* dest, DWORD range_left, DWORD range_right, wchar_t* result, DWORD* counter,
		lsJITFUNC* funcenv);
DWORD __token2code_forsee_args(
		lsEXPRTOKEN* source, lsJITcode* dest, DWORD current_loc, DWORD range_right, wchar_t* arg_1, wchar_t* arg_2,
...
void __token2code_tempvar(
		...
		lsJITFUNC* funcenv) {
	using namespace JIT;
	wcscpy(result, TEMPVAR_HEADER);
	DWORD strptr = wcslen(TEMPVAR_HEADER);
	__token2code_tempname(result, &strptr, counter);
	__log(L"######### result: ~ #########\n", result);
	dest->newvar(result);
	bool init_command = 1;
	wchar_t lastest_op = (wchar_t)I_DONT_CARE;
		...
		switch (source->tokens[i].type) {
			case TRUE_CONST: {
				...
				if (init_command) dest->setconst(result, source->tokens[i].floatval, 0, NULL);
					...
					wchar_t tempvar[MAX_VAR_NAME];
					wcscpy(tempvar, TEMPVAR_HEADER);
					strptr = wcslen(TEMPVAR_HEADER);
				...
				__log(L"Func: ~\n", source->tokens[i].name);
    			wchar_t funcname[MAX_FUNC_NAME];
    			wcscpy(funcname, source->tokens[i].name);
    			wchar_t arg_1[MAX_VAR_NAME], arg_2[MAX_VAR_NAME];
    			bool have_2_args;
    			DWORD func_end = __token2code_forsee_args(
					source, dest, i, range_right, arg_1, arg_2, counter, funcenv, &have_2_args);
    			if (wcscmp(funcname, L"sqrt") == 0) {
        			if (init_command) {
...
void token2code(lsEXPRTOKEN* source, lsJITcode* dest, lsJITFUNC* funcenv) {
	wchar_t result[JIT::MAX_VAR_NAME];
	DWORD counter = 0;
...
struct EXPR_COMPILER {
	...
	BYTE compile_expr(const wchar_t* expr) {
	...
	void config(const wchar_t* name, float val) {
...
void showcode(lsJITcode* code) {
	...
	for (DWORD i = 0; i < code->counter; i++) {
		switch (code->code[i].op) {
			case NEW: wprintf(L"New: %ls\n", code->code[i].var_source); break;
			case CALC: {
				wchar_t mode;
				switch (code->code[i].mode) {
					case ADD:  mode = L'+'; break; case SUB:  mode = L'-'; break;
					case MUL:  mode = L'*'; break; case DIV:  mode = L'/'; break;
					case SQRT: mode = L'S'; break; case COPY: mode = L'C';
				}
				wprintf(L"Calc: %ls %lc %ls\n", code->code[i].var_source, mode, code->code[i].var_dest);
				break;
			} case SETCONST: wprintf(L"Set: %ls = %.2f\n", code->code[i].var_source, code->code[i].data_f); break;
			case SETCONST_IDP:
				wprintf(L"Set(Independent): %ls = %.2f\n", code->code[i].var_addition, code->code[i].data_f); break;
...
void showtoken(lsEXPRTOKEN* tokens) {
	...
	for (DWORD i = 0; i < tokens->counter; i++) {
		switch (tokens->tokens[i].type) {
			...
			case FAKE_CONST: wprintf(L"Fake const: %ls\n", tokens->tokens[i].name); break;
			case VARNAME: wprintf(L"Variable: %ls\n", tokens->tokens[i].name); break;
			case OPERATOR: wprintf(L"OP: %c\n", tokens->tokens[i].singlechar); break;
			case FUNCNAME: wprintf(L"Function: %ls\n", tokens->tokens[i].name); break;
...
int main() {
	...
	BYTE result = compiler.compile_expr(L"log(x, y) + a + b");
	wprintf(L"LOL");
	compiler.config(L"a", 1.f);
	compiler.config(L"b", 2.f);
	wprintf(L"LOL\n");
	wprintf(L"code section:\n");
	showHEX(compiler.vars.stEXEmem.EXEmem, compiler.vars.stEXEmem.counter, 16);
	wprintf(L"const section:\n");
	showHEX(compiler.vars.stEXEmem.EXEmem + compiler.vars.code_size, compiler.vars.stack_counter * sizeof(float), 16);
	wprintf(L"LOL\n");
