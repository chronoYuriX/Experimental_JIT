...
namespace CALC {
	...
	const int32_t DEFAULT_INPUT_SIZE = 200, DEFAULT_TOOLBAR_SIZE = 20;
	const BGRA DEFAULT_INPUT_COLOR = BGRA{ 32, 32, 32 }, DEFAULT_TOOLBAR_COLOR = BGRA{ 64, 0, 0 },
		DEFAULT_CANVAS_COLOR = BGRA{ 0, 0, 0 };
}
struct CALC_WINDOW {
	HWND             window_hwnd;
	int32_t          window_size_x, window_size_y, window_input_size, window_toolbar_size;
	BYTE*            window_buffer;
	...
	BGRA             window_input_color, window_toolbar_color, window_canvas_color;
	CALC_WINDOW(int32_t __window_size_x, int32_t __window_size_y, EXPR_COMPILER_INFO* __compiler_info):
			...
			icon_big(NULL), icon_small(NULL),
			window_input_color(CALC::DEFAULT_INPUT_COLOR), window_toolbar_color(CALC::DEFAULT_TOOLBAR_COLOR),
			window_canvas_color(CALC::DEFAULT_CANVAS_COLOR),
			window_input_size(CALC::DEFAULT_INPUT_SIZE), window_toolbar_size(CALC::DEFAULT_TOOLBAR_SIZE),
    	...
    	window_BMI.bmiHeader.biSizeImage   = window_size_x * window_size_y * sizeof(BGRA);
	}
	~CALC_WINDOW() {
		async_exit(0);
	...
	HICON __generate_icon(int32_t size) {
		int32_t mask_row_size = ((size + 31) / 32) * sizeof(BGRA);
		BYTE* icon_bmp = (BYTE*)HeapAlloc(__g_hheap, 0, size * (size * sizeof(BGRA) + mask_row_size));
		BYTE* mask_bmp = icon_bmp + (size * size * sizeof(BGRA));
		float zoom = 1536.f / (size * 2.f);
		for (int32_t y = 0; y < size; y++) {
			for (int32_t x = 0; x < size; x++) {
				BGRA color = gradientRGB(uint16_t(NCM::powf(float(x * x + y * y), .5f) * zoom));
				dot(POINT{ x, y }, color, icon_bmp, size, RECT{ 0, 0, size, size });
			}
		}
		memset(mask_bmp, 0xFF, size * mask_row_size); // 1-bit, alpha = 0
		HBITMAP icon_hbmp = CreateBitmap(size, size, 1, 32, icon_bmp);
		HBITMAP mask_hbmp = CreateBitmap(size, size, 1, 1, mask_bmp);
		ICONINFO icon_info = {
			.fIcon    = TRUE,
			.xHotspot = 0,
			.yHotspot = 0,
	...
	LRESULT CALLBACK main_process(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
	    switch (msg) {
    		case WM_CREATE: {
    			icon_big = __generate_icon(32); icon_small = __generate_icon(16);
				SendMessageW(hwnd, WM_SETICON, ICON_BIG, (LPARAM)icon_big);
				SendMessageW(hwnd, WM_SETICON, ICON_SMALL, (LPARAM)icon_small);
        		...
        		if ((SIZE_T)window_size_x * window_size_y > window_buffer_xy) {
        			...
        			window_buffer = (BYTE*)HeapReAlloc(__g_hheap, 0, window_buffer, window_buffer_xy * sizeof(BGRA));
				}
				break;
			} case WM_PAINT: {
				...
				if (wparam == CALC::TIMER_RESIZE_BUFFER) {
					...
					else window_buffer = (BYTE*)HeapReAlloc(
						__g_hheap, 0, window_buffer, window_buffer_xy * sizeof(BGRA));
				}
				break;
    		...
    		case WM_DESTROY: {
            	...
            	if (icon_big) { DestroyIcon(icon_big); icon_big = NULL; }
            	if (icon_small) { DestroyIcon(icon_small); icon_small = NULL; }
	...
	DWORD WINAPI run() {
		...
		window_buffer = (BYTE*)HeapAlloc(__g_hheap, 0, window_buffer_xy * sizeof(BGRA));
		ZeroMemory(window_buffer, window_buffer_xy * sizeof(BGRA));
	...
	void ui_render_toolbar() {
		fill_rect(RECT{ 0, 0, window_size_x, window_toolbar_size }, window_toolbar_color, window_buffer, window_size_x);
	}
	void ui_render_input() {
		fill_rect(RECT{ 0, window_toolbar_size, window_input_size, window_size_y }, window_input_color, window_buffer,
			window_size_x);
	}
	void ui_clear_func() {
		fill_rect(RECT{ 0, window_toolbar_size, window_input_size, window_size_y }, window_input_color, window_buffer,
