...
struct TRIANGLE {
	float3D A, B, C;
	TRIANGLE(float3D _A, float3D _B, float3D _C): A(_A), B(_B), C(_C) { }
	TRIANGLE() = default;
};
struct FUNC_3D {
...
struct GL_WINDOW {
	...
	struct FRAME_INFO {
		float3D     rotate, rotate_backup, offset, offset_backup;
		quaternion  rotate_quaternion, rotate_quaternion_backup;
		int32_t     counter;
		GL_CHARSET* charset;
		GL_TEXT    *text, *text_replace;
		FRAME_INFO(): rotate(0.f, 0.f, 0.f), rotate_backup(0.f, 0.f, 0.f), offset(0.f, 0.f, -5.f), offset_backup(0.f, 0.f, -5.f),
			rotate_quaternion(1.f, 0.f, 0.f, 0.f), rotate_quaternion_backup(1.f, 0.f, 0.f, 0.f),
			counter(0), charset(nullptr), text(nullptr), text_replace(nullptr) { }
	};
	...
	void render_frame() {
        ...
        switch_dimension(3);
        glTranslatef(frame_info.offset.x, frame_info.offset.y, frame_info.offset.z);
        /*
        glRotatef(frame_info.rotate.x * 180.f / NCM::PI, 1.f, 0.f, 0.f);
        glRotatef(frame_info.rotate.y * 180.f / NCM::PI, 0.f, 1.f, 0.f);
        glRotatef(frame_info.rotate.z * 180.f / NCM::PI, 0.f, 0.f, 1.f);
        */
        GLfloat rotation_matrix[16];
    	frame_info.rotate_quaternion.matrixize(rotation_matrix);
		...
		if (frame_info.counter > 100) {
			glEnable(GL_TEXTURE_2D);
			frame_info.text_replace->draw(POINT{ text_range.left, text_range.top }, frame_info.charset, htextDC);
			glDisable(GL_TEXTURE_2D);
		} else {
			if (frame_info.counter == 100) frame_info.text_replace = new GL_TEXT(L"A\nBC\nDEF", htextDC);
			else if (frame_info.counter > 50) {
				glEnable(GL_TEXTURE_2D);
				frame_info.text->draw(POINT{ text_range.left, text_range.top }, frame_info.charset, htextDC);
				glDisable(GL_TEXTURE_2D);
			} else if (frame_info.counter == 50) frame_info.text = new GL_TEXT(L"ABCDEFGH", htextDC);
			frame_info.counter++;
		}
        glLineWidth(2.f);
    	...
    	if ((mouse_key_state & MOUSE_LEFT_DOWN) && (mouse_key_state & MOUSE_VALID_DOWN_POS)) {
    		...
    		glEnd();
    		/*
    		frame_info.rotate.x = frame_info.rotate_backup.x + float(mouse_current_pos.y - mouse_down_pos.y) * NCM::PI / 200.f;
    		frame_info.rotate.y = frame_info.rotate_backup.y + float(mouse_current_pos.x - mouse_down_pos.x) * NCM::PI / 200.f;
    		*/
    		float delta_x = float(mouse_current_pos.y - mouse_down_pos.y);
    		float delta_y = float(mouse_current_pos.x - mouse_down_pos.x);
    		quaternion rotate_x = quaternion::construct_from_axis(float3D(1.f, 0.f, 0.f), delta_x * NCM::PI / 200.f),
				rotate_y = quaternion::construct_from_axis(float3D(0.f, 1.f, 0.f), delta_y * NCM::PI / 200.f);
    		quaternion delta_quatation = rotate_x * rotate_y;
    		frame_info.rotate_quaternion = (delta_quatation * frame_info.rotate_quaternion_backup).align1();
		} else if (mouse_key_state & MOUSE_ACTION_LEFT_UP) {
			// frame_info.rotate_backup = frame_info.rotate;
			frame_info.rotate_quaternion_backup = frame_info.rotate_quaternion;
		}
    		...
    		glEnd();
			float2D delta(float(mouse_current_pos.x - mouse_down_pos.x) * .005, float(mouse_down_pos.y - mouse_current_pos.y) * .005);
			float3D local_offset(delta.x, delta.y, 0.f);
    		float3D world_offset = frame_info.rotate_quaternion.rotate(local_offset);
    		frame_info.offset.x = frame_info.offset_backup.x + world_offset.x;
    		frame_info.offset.y = frame_info.offset_backup.y + world_offset.y;
    		frame_info.offset.z = frame_info.offset_backup.z + world_offset.z;
		}
