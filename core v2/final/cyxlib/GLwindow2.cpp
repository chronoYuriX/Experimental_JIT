...
int SaveBMP32(const char* filename, HBITMAP hbm)
{
    BITMAP bm;
...
struct GL_TEXT {
	...
	static constexpr DWORD HEAPSTACK_BOUNDARY = 4096;
	static BITMAPINFO _create_BMI(SIZE BMPsize, WORD BMPbitcount) {
    	...
    	BMI.bmiHeader.biSizeImage   = BMPsize.cx * BMPsize.cy * ((BMPbitcount == 32) ? 4 : 3);
    	return BMI;
	}
	...
	GL_TEXT(const wchar_t* _text, HDC htextDC) {
		...
		/*
		BITMAPINFO BMI_GLread = _create_BMI(text_size, 24);
		BYTE* RGBbuffer;
		if (BMI_GLread.bmiHeader.biSizeImage > HEAPSTACK_BOUNDARY)
			RGBbuffer = (BYTE*)HeapAlloc(process_heap, 0, BMI_GLread.bmiHeader.biSizeImage);
		else RGBbuffer = (BYTE*)__builtin_alloca(BMI_GLread.bmiHeader.biSizeImage);
		GetDIBits(htextDC, *htextBMP, 0, text_size.cy, RGBbuffer, &BMI_GLread, DIB_RGB_COLORS);
		*/
		SaveBMP32("debug_out.bmp", htextBMP);
    	...
    	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, text_size.cx, text_size.cy, 0, GL_BGRA_EXT, GL_UNSIGNED_BYTE, text_data);
    	DeleteObject(htextBMP);
    	*/
    	
    	/*
    	if (BMI_GLread.bmiHeader.biSizeImage > HEAPSTACK_BOUNDARY) HeapFree(process_heap, 0, RGBbuffer);
    	*/
	}
	~GL_TEXT() { glDeleteTextures(1, &textureID); }
	void draw(POINT loc) {
		if (glIsTexture(textureID)) glBindTexture(GL_TEXTURE_2D, textureID);
        else printf("Bad texture\n");
		glBegin(GL_QUADS);
        	glTexCoord2f(0.0f, 0.0f); glVertex2f(float(loc.x), float(loc.y));
        	glTexCoord2f(1.0f, 0.0f); glVertex2f(float(loc.x + text_size.cx), float(loc.y));
        	glTexCoord2f(1.0f, 1.0f); glVertex2f(float(loc.x + text_size.cx), float(loc.y + text_size.cy));
        	glTexCoord2f(0.0f, 1.0f); glVertex2f(float(loc.x), float(loc.y + text_size.cy));
    	glEnd();
	}
};
		...
		*/
		...
		while (1) {
        	if (PeekMessageW(&msg, NULL, 0, 0, PM_REMOVE)) {
        	    ...
        	    glFlush();
        	    if (glGetError() != GL_NO_ERROR) printf("Error: %d\n", glGetError());
    			SwapBuffers(hDC);
        	    theta += .3f;
        	    Sleep(16);
        	}
