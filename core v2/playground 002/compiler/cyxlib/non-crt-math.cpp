#ifndef _NCM_CPP_INCLUDED
#define _NCM_CPP_INCLUDED
#include <stdint.h>
#include <string.h>
#define NCM_API __attribute__((used, noinline))
#ifndef _NCM_NO_PACKING
namespace NCM {
#endif
...
NCM_API float fabs(float f) { return (f > 0.f) ? f : -f; }
NCM_API double fabs(double d) { return (d > 0.) ? d : -d; }

// ### math functions ##################################################################################################
NCM_API float fact32(int32_t n) {
    float result = 1.f;
    for (int32_t i = 1; i < n; ++i) result *= i;
    return result;
}
NCM_API float normalize2PI(float x) {
    while (x > PI) x -= 2.f * PI;
    while (x < -PI) x += 2.f * PI;
    return x;
}

NCM_API float sin(float x) {
    x = normalize2PI(x);
...
NCM_API float csc(float x) {
	float s = sin(x);
	if (fabs(s) < ERR) return (s > 0.f) ? INF : -INF;
	return 1.f / s;
}
NCM_API float sec(float x) {
	float c = cos(c);
	if (fabs(c) < ERR) return (c > 0.f) ? INF : -INF;
	return 1.f / c;
}

NCM_API float ln(float x) {
    if (x <= 0.f) return -INF;
    int32_t k = 0;
    while (x >= 2.f) { x *= .5f; k++; }
...
NCM_API float log10(float x) { return ln(x) * LOG10E; }
NCM_API float log(float base, float mantissa) { return ln(mantissa) / ln(base); }

NCM_API float ipow(float base, int64_t exponent) {
...
NCM_API float pow(float base, float exponent) {
...
#endif
#endif // #ifndef _NCM_CPP_INCLUDED
