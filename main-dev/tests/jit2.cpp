...
struct DICTEX {
	...
	void add(const wchar_t* key, TYPE_VAL val) {
		if (wcslen(key) + dict.storage_counter + 2 > max_storage || dict.ptr_counter >= max_strs)
			__report_error(L"Dict overflow");
		dict.add(key, val);
	}
	TYPE_VAL get(const wchar_t* key, TYPE_VAL not_found) {
	...
	TYPE_VAL get_ptr(const wchar_t* key) {
		if (dict.ptr_counter == 0) return nullptr;
		return dict.get_ptr(key);
	}
...
// ### func generation 3 ###############################################################################################
namespace JIT {
	const BYTE AT_REG = 1, AT_STACK = 2, NOT_FOUND = 3, FREE = 4, OCCUPIED = 5, FULL = ~0, MAX_REGS = 8;
	const DWORD I_DONT_CARE = ~0, MAX_VARS = 64, MAX_CONSTS = 64, AVERAGE_VAR_NAME_LEN = 16, MAX_CODE = 64;
}
struct JITvar {
	BYTE regID, state;
	DWORD stackID;
	JITvar(BYTE state, BYTE regID, DWORD stackID): state(state), regID(regID), stackID(stackID) { }
	JITvar(): state(BYTE(JIT::I_DONT_CARE)), regID(BYTE(JIT::I_DONT_CARE)), stackID(JIT::I_DONT_CARE) { }
};

namespace JIT {
	const JITvar JITvar_NOT_FOUND(NOT_FOUND, BYTE(I_DONT_CARE), I_DONT_CARE);
	const BYTE NEW = 1, CALC = 2, SETCONST = 3, SETCONST_IDP = 4, CALLFUNC = 5, FINISH = 6;
	const char VOID_NAME[] = "__void";
}
struct JITcode {
	BYTE  op, mode;
	char  var_source[JIT::AVERAGE_VAR_NAME_LEN], var_dest[JIT::AVERAGE_VAR_NAME_LEN], var_addition[JIT::AVERAGE_VAR_NAME_LEN];
	union {
		float data_f;
		DWORD data_i;
		void* funcptr;
...
struct lsJITvar {
	...
	DWORD          code_size, const_size, stack_size, const_counter, stack_counter;
	DICTEX<JITvar> vars;
	EXEMEM_STRUCT  stEXEmem;
	lsJITvar(DWORD code_size, DWORD stack_size, DWORD max_vars, DWORD max_consts): const_counter(0), stack_counter(0),
			code_size(code_size), stack_size(stack_size),
			stEXEmem(code_size + max_consts * sizeof(float)),
			vars(max_vars, max_vars * AVERAGE_VAR_NAME_LEN) {
		using namespace JIT;
		vars.add("x", JITvar(AT_REG, 0, I_DONT_CARE));
		vars.add("y", JITvar(AT_REG, 1, I_DONT_CARE));
	...
	JITvar* find_var(const char* name) {
		JITvar target = vars.get_ptr(name);
		if (target) return tatget;
		__report_error(L"Undefined variable name"); return NULL;
	}
	BYTE swap(BYTE protected_regID) {
		using namespace JIT;
		for (DWORD i = 0; i < var_counter; i++) {
			JITvar* current_var = vars.dict.vals + i;
			if (current_var->state == AT_REG &&
					(protected_regID == I_DONT_CARE || current_var->regID != protected_regID)) {
				if (current_var->stackID == I_DONT_CARE) current_var->stackID = stack_counter++;
				__log(L"(Swap ~-reg$ to stack$)\n", vars.dict.strptrs_original[i], current_var->regID,
					current_var->stackID);
				memOP(stEXEmem, current_var->regID, current_var->stackID * sizeof(float), SAVE);
				current_var->state = AT_STACK;
				regs[current_var->regID] = FREE;
				return current_var->regID;
			}
		}
	...
	BYTE swap_back(const char* name, BYTE protected_regID) {
		using namespace JIT;
		JITvar* source = find_var(name);
		if (source->state == AT_REG) {
...
struct PRE_COMPILE_VAR {
	char name[JIT::AVERAGE_VAR_NAME_LEN];
	DWORD start, end;
};
struct IDPCONST {
	char name[JIT::AVERAGE_VAR_NAME_LEN];
...
struct EXPRTOKEN {
	...
	union {
		float floatval;
		char name[__token_maxconst(JIT::MAX_FUNC_NAME, JIT::AVERAGE_VAR_NAME_LEN)];
		char singlechar;
	};
};
struct lsEXPRTOKEN {
...
void expr2token(const char* expr, lsEXPRTOKEN* dest, lsIDPCONST* changeables) {
	...
	for (DWORD i = 0; i < exprlen; i++) {
		...
		if (__expr2token_isnum(expr[i])) {
			...
			char name[AVERAGE_VAR_NAME_LEN]; memcpy(name, expr + i, namelen); name[namelen] = '\0';
...
namespace JIT { const char TEMPVAR_HEADER[AVERAGE_VAR_NAME_LEN] = "__tmp"; }
...
DWORD __token2code_forsee_args(
		...
		DWORD* counter, lsJITFUNC* funcenv) {
	using namespace JIT;
		...
		switch (source->tokens[i].type) {
			case TRUE_CONST: {
				...
				if (init_command) dest->setconst(result, source->tokens[i].floatval, 0, NULL);
					...
					char tempvar[AVERAGE_VAR_NAME_LEN];
    			...
    			char arg_1[AVERAGE_VAR_NAME_LEN], arg_2[AVERAGE_VAR_NAME_LEN];
    			DWORD func_end = __token2code_forsee_args(source, dest, i, range_right, arg_1, arg_2, counter, funcenv);
...
void token2code(lsEXPRTOKEN* source, lsJITcode* dest, lsJITFUNC* funcenv) {
	char result[JIT::AVERAGE_VAR_NAME_LEN];
	DWORD counter = 0;
...
int main() {
	...
	if (result == JIT::PASS) wprintf(L"compile: 1 + 2 = %.2f (expect 3.00)\n", compiler.run(1.f, 2.f));
