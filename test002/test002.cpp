...
void stackOP(EXEMEM_STRUCT& sEXEmem, BYTE operation, DWORD size) {
	using namespace JIT;
	size = (size + 8 + 15) & ~15; // Compensate the return address, then align to 16 bytes
...
BYTE* compile(const char* code, DWORD vercount, DWORD EXEsize, DWORD constsize, DWORD stacksize) {
	using namespace JIT;
...
BYTE* compile2(const char* code, DWORD vercount, DWORD EXEsize, DWORD constsize, DWORD stacksize) {
	using namespace JIT;
