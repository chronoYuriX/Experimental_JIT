...
// ### tests ###########################################################################################################
__attribute__((noinline)) float fake_log2F_F(float base, float mantissa) {
	if (base == 2.f && mantissa == 16.f) return 4.f;
	return 0.f;
}
__attribute__((noinline)) float fake2_log2F_F(float base, float mantissa) {
	float result = fake_log2F_F(base, mantissa);
	float unused = fake_log2F_F(mantissa, base);
	for (int i = 0; i < 20; i++) unused *= 1.1f;
	result += unused; result -= unused;
	return result;
}
__attribute__((noinline)) float fake3_log2F_F(float base, float mantissa) {
	float result = fake2_log2F_F(base, mantissa);
	return result;
}
void test_call() {
	using namespace JIT;
	...
	code.callfunc((void*)fake3_log2F_F, "x", "x", "n");
	code.finish("x");
...
void test_call_iter() {
	...
	wprintf(L"compile: 1 + 2 = %.2f (expect 3.00)\n", compiler.run(1.f, 2.f));
}
