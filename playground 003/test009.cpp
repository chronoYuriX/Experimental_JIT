...
struct lsJITvar {
	...
	lsJITvar(DWORD code_size, DWORD stack_size, DWORD max_vars, DWORD max_consts): var_counter(2), const_counter(0),
			...
			vars((JITvar*)VirtualAlloc(NULL, max_vars * sizeof(JITvar), MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE)) {
		using namespace JIT;
	...
	~lsJITvar() {
		...
		if (vars) VirtualFree(vars, 0, MEM_RELEASE);
...
struct lsIDPCONST {
	...
	lsIDPCONST(DWORD max_consts): counter(0),
		consts((IDPCONST*)VirtualAlloc(NULL, max_consts * sizeof(IDPCONST), MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE))
		{ }
	~lsIDPCONST() {
...
void pre_compile(const lsJITcode* source, lsJITvar* dest, DWORD max_vars) {
	...
	PRE_COMPILE_VAR* vars_pre = (PRE_COMPILE_VAR*)VirtualAlloc(
		NULL, dest->max_vars * sizeof(PRE_COMPILE_VAR), MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
			...
			case NEW: {
				...
				if (source->code[i].op == SETCONST_IDP) {
					if (changeables != NULL) changeables->append(
						source->code[i].var_addition, dest->load_const(regID, NAN));
...
namespace JIT {
	...
	float math_log2F_F(float base, float mantissa) { return logf(mantissa) / logf(base); }
...
struct lsJITFUNC {
	...
	lsJITFUNC(DWORD max_funcs, bool avtivate_builtins): max_funcs(max_funcs), counter(0),
			funcs((JITFUNC*)VirtualAlloc(NULL, max_funcs * sizeof(JITFUNC), MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE)) {
		using namespace JIT;
		if (avtivate_builtins) {
			append("sin", (void*)math_sin); append("cos", (void*)math_cos); append("tan", (void*)math_tan);
			append("abs", (void*)math_abs); append("log", (void*)math_log2F_F); append("log1", (void*)math_log);
		}
	}
	~lsJITFUNC() {
		if (funcs) VirtualFree(funcs, 0, MEM_RELEASE);
		funcs = NULL;
	}
...
struct EXPR_COMPILER {
	...
	float run(float x, float y) {
		if (main_executable) return main_executable(x, y);
		return NAN;
	}
};
...
// ### tests ###########################################################################################################
float fake_log2F_F(float base, float mantissa) {
	if (base == 2.f && mantissa == 16.f) return 4.f;
	return 0.f;
}
float fake2_log2F_F(float base, float mantissa) {
	logf(base); logf(mantissa);
	if (base == 2.f && mantissa == 16.f) return 4.f;
	return 0.f;
}
void test_call() {
	...
	code.newvar("n");
	code.setconst("n", 25.f, 0, NULL);
	code.calc(SUB, "n", "y");
	// code.callfunc((void*)math_log2F_F, "x", "x", "n");
	code.callfunc((void*)fake_log2F_F, "x", "x", "n");
	// code.callfunc((void*)fake2_log2F_F, "x", "x", "n");
	code.finish("x");

	BYTE* EXEmem = compile(&code, &vars, NULL);
	...
	wprintf(L"compile: log(x:2, 25 - y:9) = %.2f (expect 4.00)\n", func(2.f, 9.f));
}
void test_call_iter() {
	EXPR_COMPILER_INFO info = {
		.max_tokens = 256,
...
void test_expr_call() {
	...
	lsJITcode   code(256);
	lsJITvar    vars(1024, 256, 64, 64);
	lsJITFUNC   funcenv(16, 1);
	expr2token("log(3, x) + y", &tokens, &consts);
	__showlog_once;
	showtoken(&tokens);
	__showlog_once;
	token2code(&tokens, &code, &funcenv);
	...
	showcode(&code);
	__showlog_once;
	BYTE* EXEmem = compile(&code, &vars, &consts);
	__showlog_once;
	func2F_F func = (func2F_F)EXEmem;
	wprintf(L"%p --> %.2f\n", math_log2F_F, math_log2F_F(3.f, 81.f));
	wprintf(L"Code section:\n"); showHEX(EXEmem, vars.stEXEmem.counter, 16);
	wprintf(L"Const section:\n"); showHEX(EXEmem + vars.code_size, vars.const_counter * sizeof(float), 16);
	wprintf(L"compile: log(3, x:9) + y:10 = %.2f (expect 12.00)\n", func(9.f, 10.f));
}
